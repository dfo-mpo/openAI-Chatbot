...
import CSV_PDF_Analyzer_App.models as model
...
def giphy_image(request):
    image_path = os.path.join(settings.BASE_DIR, 'imgs', 'giphy.gif')
    with open(image_path, 'rb') as f:
        return HttpResponse(f.read(), content_type="image/gif")

def giphy_image2(request):
    image_path = os.path.join(settings.BASE_DIR, 'imgs', 'giphy2.gif')
    with open(image_path, 'rb') as f:
        return HttpResponse(f.read(), content_type="image/gif")

def giphy_image3(request):
    image_path = os.path.join(settings.BASE_DIR, 'imgs', 'giphy3.gif')
    with open(image_path, 'rb') as f:
        return HttpResponse(f.read(), content_type="image/gif")
